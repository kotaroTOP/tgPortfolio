...
@bot.message_handler(commands=['new_project']) # принятие команды создать новый проект
def addtask_command(message):
    bot.send_message(message.chat.id, "Введите название проекта:")
    bot.register_next_step_handler(message, name_project)

def name_project(message): # имя проекта
    name = message.text
...
def link_project(message, data): # ссылка проекта
...
def callback_project(message, data, statuses): # Выбран не нужный статус или нужный
    status = message.text
...
@bot.message_handler(commands=['skills']) # принятие команды установить скилл
...
def skill_project(message, projects): # добавить скилл прокета
...
def set_skill(message, project_name, skills): # процесс  установления скилла
    skill = message.text
    user_id = message.from_user.id
    if message.text == cancel_button:
        cansel(message)
...
@bot.message_handler(commands=['projects']) # посмотреть проекты
...
@bot.callback_query_handler(func=lambda call: True) # Я не знаю что это
def callback_query(call):
    project_name = call.data
    info_project(call.message, call.from_user.id, project_name)


@bot.message_handler(commands=['delete']) # принятие команды удалить проект
...
def delete_project(message, projects): # процесс удаление проекта
...
@bot.message_handler(commands=['update_projects']) # начало обновления проекта
...
def update_project_step_2(message, projects): # стадия обновления 2
    project_name = message.text
    if message.text == cancel_button:
        cansel(message)
        return
...
def update_project_step_3(message, project_name): # стадия обновления 3
...
def update_project_step_4(message, project_name, attribute): # стадия обновления 4
...
@bot.message_handler(func=lambda message: True) #* Что то неправильное введено
...
#~ Запуск бота и создание менеджера.
if __name__ == '__main__':
    manager = DB_Manager(DATABASE)
    bot.infinity_polling(print("Bot started"))
#TODO сделать более развернутые комментарии.